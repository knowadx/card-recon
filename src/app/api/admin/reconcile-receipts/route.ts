 ...
 * anúncio real via MetaReceipt, e classifica:
 ...
 *   sem_recibo    → metaRef sem recibo correspondente (suspeito / falta importar recibo)
...
export async function GET() {
  ...
  const [receipts, controlledAccts, bank] = await Promise.all([
    ...
    prisma.transaction.findMany({ where: { metaRef: { not: null } }, select: { metaRef: true, amount: true, date: true, cardLast4: true, account: { select: { company: { select: { name: true } } } } } }),
  ]);
  ...
  return NextResponse.json({
    ...
    vazamentoSuspeito: leak,         // recibo aponta p/ conta NÃO controlada
    semReciboAmostra: semReciboSample, // metaRef sem recibo (importar recibo do mês, ou suspeito)
  });
}
