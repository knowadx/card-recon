...
export async function GET(request: Request) {
  ...
  const [charges, receipts] = await Promise.all([
    prisma.metaBillingCharge.findMany({
      ...
      select: { transactionId: true, accountId: true, accountName: true, bmName: true, chargedAt: true },
    }),
    prisma.metaReceipt.findMany({ select: { transactionId: true } }),
  ]);
  const haveReceipt = new Set(receipts.map((r) => r.transactionId));

  type Acc = { accountId: string; name: string | null; bm: string | null; total: number; comPdf: number; semPdf: number; semPdfPorMes: Record<string, number> };
  const byAcct = new Map<string, Acc>();
  let totalSemPdf = 0;

  for (const c of charges) {
    let a = byAcct.get(c.accountId);
    if (!a) { a = { accountId: c.accountId, name: c.accountName, bm: c.bmName, total: 0, comPdf: 0, semPdf: 0, semPdfPorMes: {} }; byAcct.set(c.accountId, a); }
    a.total++;
